...
class TestDendrogram(unittest.TestCase):
    ...
    def test_coords(self):
        ...
        edge_exp = pd.DataFrame(
            {
                'a': [
                    'a', 'f', 141.35398602846797, 339.46141862722482, 83.371774496551481, 292.50834951934343], 'b': [
                    'b', 'g', 215.86090210071345, 343.36616063979909, 254.48144795927647, 487.5], 'c': [
                    'c', 'h', 403.57843531045097, 221.46096919708964, 478.08535138269644, 225.36571120966394], 'd': [
                        'd', 'h', 403.57843531045097, 221.46096919708964, 483.79103611135702, 12.500000000000028], 'e': [
                            'e', 'f', 141.35398602846797, 339.46141862722482, 16.20896388864297, 420.73154625569776], 'f': [
                                'f', 'g', 215.86090210071345, 343.36616063979909, 141.35398602846797, 339.46141862722482], 'g': [
                                    'g', 'i', 278.43341317062595, 302.73109682556259, 215.86090210071345, 343.36616063979909], 'h': [
                                        'h', 'i', 278.43341317062595, 302.73109682556259, 403.57843531045097, 221.46096919708964]}, index=[
                                            'Node id', 'Parent id', 'px', 'py', 'x', 'y']).T
        node_exp = pd.DataFrame(
            {
                'a': [
                    'a', 83.371774496551481, 292.50834951934343], 'b': [
                    'b', 254.48144795927647, 487.5], 'c': [
                    'c', 478.08535138269644, 225.36571120966394], 'd': [
                        'd', 483.79103611135702, 12.500000000000028], 'e': [
                            'e', 16.20896388864297, 420.73154625569776], 'f': [
                                'f', 141.35398602846797, 339.46141862722482], 'g': [
                                    'g', 215.86090210071345, 343.36616063979909], 'h': [
                                        'h', 403.57843531045097, 221.46096919708964], 'i': [
                                            'i', 278.43341317062595, 302.73109682556259]}, index=[
                                                'Node id', 'x', 'y']).T
        node_res, edge_res = t.coords(500, 500)
        pdt.assert_frame_equal(node_exp, node_res)
        pdt.assert_frame_equal(edge_exp, edge_res)
